...
public class EmployeeGenderReader implements IConsoleReader {
	 ...
	 * 
	 * @author 別所大空
	 * @return Object(int) 性別（0:回答なし 1:男性 2:女性 9:その他）
	 * @throws IllegalInputException 不正な入力
	 * @throws SystemErrorException	 システムエラー
	 */
	public static Object employeeGenderReader() throws IllegalInputException, SystemErrorException {
		...
		try {
			...
			input = br.readLine();
			if (!employeeGenderReader.isValid(input)) {
				String errorMessage = employeeGenderReader.getErrorMsg();
				throw new IllegalInputException(errorMessage);
			}
			if (employeeGenderReader.isParseInt()) {
	 ...
	 * @throws SystemErrorException	 システムエラー
	 */
	public static Object employeeGenderReader(String message) throws IllegalInputException, SystemErrorException {
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		EmployeeGenderReader employeeGenderReader = new EmployeeGenderReader();
		String input = null;
		try {
	...
	public String getErrorMsg() {
		return ConstantMsg.MSG_VAILD_GENDER;
	}
	...
	public boolean isValid(String inputString) {
		...
		//正規表現チェック
		Pattern p = Pattern.compile(ConstantMsg.VAILD_PATTERN_GENDER);
